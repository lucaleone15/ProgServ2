...
// Récupérer la tâche uniquement si elle appartient à l'utilisateur
$task = $tasksManager->getTaskByIdAndUserId($id, $userId);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        try {
            ...
            $success = $tasksManager->updateTask($id, $updatedTask, $userId);

            if ($success) {
                header("Location: view.php?id=" . $id);
                exit();
            } else {
                $errors[] = __t('edit.error_unauthorized');
            }
        } catch (Exception $e) {
